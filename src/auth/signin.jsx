...
const Signin = () => {
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      setLoading(false); // ✅ Release button after request completes
    }
  };
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data?.email) {
        ...
        await sendOtp(); // this will also respect loading lock
        setStep("otp");
      } else {
        alert("Sign-in failed. Email not recognized.");
      }
  ...
  const handleVerifyOtp = (e) => {
    ...
    localStorage.setItem("user", JSON.stringify({ email, userId, chapter }));
      ...
      <div className="flex flex-grow items-center justify-center px-4 py-10 mt-[-130px]">
        ...
        >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 transition flex items-center justify-center disabled:opacity-70"
          >
            {loading ? (
